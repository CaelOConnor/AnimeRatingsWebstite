...
describe('searchAnimeByTitle', () => {
  ...
  describe('matching', () => {
    it('returns a row that exactly matches the query', async () => {
      await createTestAnime({ tmdbId: 99991, title: 'Fullmetal Alchemist' });
    ...
    it('returns multiple rows when several titles match', async () => {
      await createTestAnime({ tmdbId: 99991, title: 'Fullmetal Alchemist' });
      await createTestAnime({ tmdbId: 99992, title: 'Fullmetal Alchemist: Brotherhood' });
    ...
    it('does not return rows that do not match the query', async () => {
      await createTestAnime({ tmdbId: 99991, title: 'Fullmetal Alchemist' });
      await createTestAnime({ tmdbId: 99992, title: 'Attack on Titan' });
  ...
  // -------------------------------------------------------------------------
  // original_title is not searched
  // Per the handoff doc: ILIKE on title column only, not original_title.
  // -------------------------------------------------------------------------
  ...
  describe('original_title is not searched', () => {
    it('does not match against original_title', async () => {
      ...
      expect(results.some((r) => r.tmdb_id === 99991)).toBe(false);
    });
  });
